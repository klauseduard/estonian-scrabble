...
class TestWordValidation(unittest.TestCase):
    ...
    def test_blocked_stems_remove_whole_family(self):
        ...
        """
        self.assert_invalid(["õul", "õule", "õult", "õuke", "õuga"])
        self.assert_valid(["õu", "õue", "õuel", "õuega"])
        # Forms also generated by legitimate entries are unaffected
        self.assert_valid(["õuna", "öid"])

    def test_oud_horror_noun_restored_via_extra_words(self):
        """'õud' (horror) is a real word missing from upstream.

        It previously validated only by accident through the bogus
        'õuga' anchor; now it is properly allowlisted with the 'laud'
        paradigm (laud:laua:lauda), full inflection included.
        """
        self.assert_valid(["õud", "õua", "õuda", "õual", "õuast", "õuad", "õudade", "õudu"])
