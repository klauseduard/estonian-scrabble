...
class TestWordValidation(unittest.TestCase):
    ...
    def test_oud_paradigm_matches_eki(self):
        """'õud' (horror) declines <22e: õu, õudu> per EKSS/ÕS.

        Regression for a two-step misdiagnosis: ÕUL was reported as an
        illegal play and the 'õuga' anchor was removed as bogus — but
        EKI shows genitive 'õu', so ÕUL is the real adessive and 'õuga'
        the real comitative. The laud-model allowlist entries added in
        between (õua, õuda, õual) were themselves wrong. Everything is
        back to the upstream paradigm.
        """
        self.assert_valid(
            ["õud", "õu", "õudu", "õude", "õudusid", "õul", "õule", "õus", "õust", "õuga"]
        )
        # The laud-model forms briefly allowlisted by mistake are not words
        self.assert_invalid(["õua", "õuda", "õual", "õudade"])
        # Neighbouring real words are unaffected
        self.assert_valid(["õue", "õuel", "õuega", "õuna", "öid"])
