...
class WordValidator:
    ...
    def _is_connected_to_existing(
        self,
        board: List[List[Optional[str]]],
        current_turn_tiles: Set[Tuple[int, int]],
    ...
    def _are_turn_tiles_connected(
        self, board: List[List[Optional[str]]], current_turn_tiles: Set[Tuple[int, int]]
    ) -> bool:
        ...
        for row, col in current_turn_tiles:
            ...
            for word, positions in words:
                if word not in all_words:
                    ...
                    self.logger.debug(
                        f"Validating word '{word}': {'valid' if is_valid else 'invalid'}"
                    )
